...
function updateSeatsButtons() {
    ...
    console.log(seats)
    for (let y = 0; y < rows; y++) {
        containerSeats.innerHTML += '<div class="container_seats_row">'

        for (let x = 0; x < cols; x++) {
            if (seats[y][x] === 'w') {
                containerSeats.innerHTML += '<button class="container_seats_seat seat_empty" onclick = "reservarte(' + y + ',' + x + ') ">' + (y + 1) + '/' + (x + 1) + '</button>'
            }
            else if (seats[y][x] === 'e') {
                containerSeats.innerHTML += '<button class="container_seats_seat seat_chosen" onclick = "reservarte(' + y + ',' + x + ') ">' + (y + 1) + '/' + (x + 1) + '</button>'
            }
            else {
                containerSeats.innerHTML += '<button class="container_seats_seat seat_taken">' + (y + 1) + '/' + (x + 1) + '</button>'
            }
        }
        containerSeats.innerHTML += '</div>'
    }
}
